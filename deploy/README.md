...
- **主流程（数据同步 + 8 个单标的回测 + 当日信号汇总 + 日报推送）和动量轮动
  都跑在 `ghcr.io/lc4t/trading-2.0:latest` 镜像上**（`2.0` 现在是默认/稳定
  分支）。
- **动量轮动是独立的、`continue-on-error: true` 的步骤**，挂了也不影响
  日报和部署。
- 前端从 `2.0` 分支 checkout（含 `/rotation` 页面），构建后部署到
  Cloudflare Pages。
- **部署用 `--branch=dev`**：Cloudflare Pages 项目 `trading` 的「生产分支」是
  `dev`，自定义域名 `trading.sakanano.moe` 只服务生产部署。wrangler 必须显式
  `--branch=dev` 才会更新生产域名；传别的分支名（如 `1.0`/`2.0`）只会生成
